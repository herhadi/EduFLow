...
Job `teacher:reminder:before-class` dibuat saat `DailyAgenda` digenerate dan juga disiapkan ulang untuk agenda 14 hari ke depan ketika backend start. Job berjalan 5 menit sebelum jam mulai agenda berdasarkan zona waktu sekolah (`SCHOOL_TIMEZONE_OFFSET_MINUTES`, default `420` untuk WIB).

Worker tidak langsung mengirim pesan dari job agenda tersebut. Worker membaca ulang agenda pada tanggal itu, mencari agenda paling awal untuk guru yang sama, lalu hanya membuat satu `NotificationLog` Telegram dengan dedupe `teacher.reminder.before-class.{teacherId}.{tanggal}`. Jika guru memiliki jadwal berurutan seperti jam ke-3 dan jam ke-4, reminder hanya terkirim sebelum jam ke-3. Jika agenda punya guru pengganti, reminder dikirim ke guru pengganti.
