    ...
    <Card>
      ...
      <div className="mt-4 grid gap-2">
        {(assessment.scores ?? []).map((score) => (
          <div className="grid gap-2 rounded-2xl border border-blue-50 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900 md:grid-cols-[1.3fr_120px_1fr]" key={score.id}>
            <div className="min-w-0">
              <p className="truncate text-sm font-black text-slate-900 dark:text-slate-100">{score.student.name}</p>
              <p className="mt-1 text-xs font-semibold text-muted">NIS: {score.student.nis ?? '-'}</p>
            </div>
            <input
              className={`${fieldClass} rounded-xl py-2 font-black disabled:bg-slate-100 dark:disabled:bg-slate-800`}
              disabled={!editable}
            ...
            />
            <input
              className={`${fieldClass} rounded-xl py-2 disabled:bg-slate-100 dark:disabled:bg-slate-800`}
              disabled={!editable}
