...
export function ScheduleEditorForm({
  ...
  return (
    <form
      className="w-full min-w-0 max-w-full overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 dark:border-[var(--border)] dark:bg-[var(--surface-solid)] sm:p-6"
      onSubmit={onSubmit}
    >
      <div>
      ...
      <div className="mt-5 grid min-w-0 gap-3 sm:mt-6 sm:gap-4">
        ...
        {editingId && editingSchedule?.revisions?.length ? (
          <div className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4 dark:border-amber-400/20 dark:bg-amber-400/10">
            <p className="text-xs font-black tracking-[0.1em] text-amber-800 uppercase">
              Histori Revisi
            </p>
            <div className="mt-3 space-y-2">
              {editingSchedule.revisions.map((revision) => (
                <div className="rounded-xl bg-white p-3 text-xs font-semibold text-slate-700 dark:bg-[var(--surface-solid)] dark:text-[var(--text-soft)]" key={revision.id}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-black text-slate-900 dark:text-[var(--text)]">
                    ...
                    <Button
                      className="border-red-100 bg-red-50 text-[11px] text-red-700 hover:bg-red-100 hover:text-red-800 dark:border-red-400/20 dark:bg-red-400/10 dark:text-red-100"
        ...
        />
        <p className="-mt-1 rounded-2xl bg-blue-50 px-3 py-2.5 text-xs font-semibold leading-5 text-brand-700 dark:bg-blue-400/10 dark:text-blue-100 sm:-mt-2 sm:px-4 sm:py-3">
          Mata pelajaran mengikuti pilihan guru pengampu. Guru dengan dua mapel tampil sebagai dua opsi terpisah.
        </p>

        <div className="min-w-0 rounded-2xl border border-blue-100 bg-blue-50/50 p-3 dark:border-blue-400/20 dark:bg-blue-400/10 sm:p-4">
          <p className="text-sm font-black text-slate-800 dark:text-[var(--text)]">Pilih Kelas</p>
          ...
          <div className="mt-3 grid grid-cols-[repeat(auto-fit,minmax(4.25rem,1fr))] gap-2">
            {availableGrades.map((grade) => (
              <button
                className={selectedGrade === grade ? 'min-w-0 rounded-xl bg-brand-600 px-2 py-2.5 text-sm font-black text-white sm:px-3 sm:py-3' : 'min-w-0 rounded-xl border border-blue-100 bg-white px-2 py-2.5 text-sm font-black text-brand-700 dark:border-blue-400/20 dark:bg-[var(--surface-solid)] dark:text-blue-100 sm:px-3 sm:py-3'}
                key={grade}
        ...
        <div className="min-w-0 rounded-2xl border border-blue-100 bg-blue-50/50 p-3 dark:border-blue-400/20 dark:bg-blue-400/10 sm:p-4">
          ...
          <div className="mt-3 space-y-2">
            {dayTimeSlots.map((slot) => slot.type === 'BREAK' || slot.type === 'RELIGIOUS' ? (
              <div className="rounded-xl bg-amber-50 p-3 dark:bg-amber-400/10" key={slot.id}>
                <div className="flex min-w-0 items-center justify-between gap-3 text-xs font-bold text-amber-900 dark:text-amber-100">
                ...
                {slot.type === 'RELIGIOUS' ? <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {(['BREAK', 'RELIGIOUS'] as const).map((type) => {
                    ...
                    return (
                      <button
                        className={`min-w-0 rounded-lg px-2 py-2 text-[11px] font-bold leading-4 ${currentType === type ? 'bg-brand-600 text-white' : 'bg-white text-slate-700 dark:bg-[var(--surface-solid)] dark:text-[var(--text)]'}`}
                        key={type}
                  ...
                  <p className="mt-2 rounded-lg bg-white px-3 py-2 text-[11px] font-bold text-slate-700 dark:bg-[var(--surface-solid)] dark:text-[var(--text)]">
                    Istirahat
                  </p>
                )}
              </div>
            ) : (
              <div className="min-w-0 rounded-xl bg-white p-3 dark:bg-[var(--surface-solid)]" key={slot.id}>
                <button
                  className="grid w-full min-w-0 grid-cols-1 items-center gap-1 text-left text-xs font-bold text-slate-700 dark:text-[var(--text)] sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-2"
                  disabled={!slot.isAssignable}
                  onClick={() => toggleTimeSlotPanel(slot.id)}
                  type="button"
                >
                  <span className="min-w-0 truncate">{slot.name}</span>
                  <span className="flex min-w-0 items-center gap-2 text-xs text-slate-500 dark:text-[var(--text-soft)] sm:shrink-0 sm:whitespace-nowrap">
                ...
                {slot.isAssignable && expandedTimeSlotIds.includes(slot.id) ? (
                  <div className="mt-3 grid grid-cols-[repeat(auto-fit,minmax(3rem,1fr))] gap-2 sm:flex sm:flex-wrap">
                    {gradeClasses.map((schoolClass) => {
                      ...
                      return (
                        <button
                          className={active ? 'min-w-0 rounded-lg bg-emerald-600 px-2 py-2 text-xs font-black text-white sm:min-w-11 sm:px-3' : 'min-w-0 rounded-lg border border-blue-100 bg-blue-50/50 px-2 py-2 text-xs font-black text-slate-700 dark:border-blue-400/20 dark:bg-blue-400/10 dark:text-blue-100 sm:min-w-11 sm:px-3'}
                          key={schoolClass.id}
                    ...
                    {!gradeClasses.length ? (
                      <p className="w-full rounded-lg bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600 dark:bg-[var(--surface-soft)] dark:text-[var(--text-soft)]">
      ...
      {message ? (
        <p className="mt-4 rounded-xl bg-slate-50 p-3 text-sm text-slate-700 dark:bg-[var(--surface-soft)] dark:text-[var(--text)]">
          {message}
        </p>
      ) : null}
    </form>
